...
@Component({
  selector: 'app-objective-categories-page',
  standalone: true,
  ...
  templateUrl: './objective-categories.page.html'
})
export class ObjectiveCategoriesPage implements OnInit {
  ...
  selectedCategory = signal<ObjectiveCategoryDto | null>(null);
  formTitle = 'Add Objective Category';
  ...
  openAddForm(): void {
    ...
    this.formTitle = 'Add Objective Category';
    this.isFormOpen.set(true);
  }

  openEditForm(category: ObjectiveCategoryDto): void {
  ...
  async handleFormSubmit(formData: any): Promise<void> {
    ...
    try {
      if (selected) {
        ...
        this.ns.success('Categoría actualizada', 'Categorías de Objetivos');
      } else {
        // Create new category
        await this.categoriesService.createCategory(formData);
        this.ns.success('Categoría creada', 'Categorías de Objetivos');
      }

      await this.loadCategories();
      this.closeForm();
    } catch (err: any) {
      console.error('Failed to save objective category:', err);
  ...
  openDeleteConfirm(category: ObjectiveCategoryDto): void {
    this.selectedCategory.set(category);
  ...
  async handleDeleteConfirm(): Promise<void> {
    ...
    try {
      ...
      this.ns.success('Categoría eliminada', 'Categorías de Objetivos');
      this.categories.update(list => list.filter(c => c.id !== categoryToDelete.id));
    } catch (err: any) {
      console.error('Failed to delete objective category:', err);
